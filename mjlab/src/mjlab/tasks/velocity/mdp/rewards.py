...
def track_base_height(
    ...
    asset: Entity = env.scene[asset_cfg.name]
    command = env.command_manager.get_command(command_name)  # 获得目标高度command
    assert command is not None, f"Command '{command_name}' not found."
    ...
    error = torch.abs(command[:, 0] - actual_height)
    # 为了避免梯度爆炸，使用平方误差
...
def track_angular_velocity(
    ...
    z_error = (command[:, 2] - actual[:, 2]) ** 2
    xy_error = torch.sum(actual[:, :2] ** 2, dim=1)
...
class upright:
    ...
    def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRlEnv):
        self._terrain_sensor_names: tuple[str, ...] | None = cfg.params.get("terrain_sensor_names")
    ...
    def debug_vis(self, visualizer: DebugVisualizer) -> None:
        ...
        if self._asset_cfg.body_ids:
            body_quat_w = asset.data.body_link_quat_w[:, self._asset_cfg.body_ids, :].squeeze(1)
        else:
            body_quat_w = asset.data.root_link_quat_w
        up_local = torch.tensor([0.0, 0.0, 1.0], device=env.device).expand_as(body_quat_w[:, :3])
